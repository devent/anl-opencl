...
REAL value_noise2D(vector2 v, uint seed, interp_func interp) {
	int x0 = fast_floor(v.x);
	int y0 = fast_floor(v.y);

	int x1 = x0 + 1;
	int y1 = y0 + 1;

	REAL xs = interp((v.x - (REAL) x0));
	REAL ys = interp((v.y - (REAL) y0));

	return interp_XY_2(v, xs, ys, x0, x1, y0, y1, seed, value_noise_2);
}
