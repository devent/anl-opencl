 ...
 */
/*
 * opencl_simpleRidgedLayer3_test.cpp
 *
 * Flag to run only this tests:
 * --gtest_filter="opencl_simpleRidgedLayer3_test*"
 *
...
class value_simpleRidgedLayer3_fixture: public OpenCL_Context_Buffer_Fixture {
protected:
...
TEST_P(value_simpleRidgedLayer3_fixture, show_image) {
	auto t = GetParam();
...
INSTANTIATE_TEST_SUITE_P(opencl_simpleRidgedLayer3_test, value_simpleRidgedLayer3_fixture,
		Values(
				KernelContext("simpleRidgedLayer3_with_value_noise3D_noInterp_norot_test",
						R"EOT(
...
global float *output) {
	...
	output[id] = simpleRidgedLayer3(input[id],
		...
		1, 0.125, false, 0.0, 0.0, 0.0, 0.0);
						...
						size), //
				KernelContext("simpleRidgedLayer3_with_value_noise3D_noInterp_rot_test",
						R"EOT(
